...
widthInput.addEventListener("change", function() {
    if (!activeImg) return;
    heightVal = aspectRadio.checked ?
        widthInput.value / originalWid2HeighRatio :
        heightVal;
...
heightInput.addEventListener("change", function() {
    if (!activeImg) return;
    widthVal = aspectRadio.checked ?
        originalWid2HeighRatio * heightInput.value :
        heightInput.value;
    resizeTo(widthVal, heightInput.value);
    alert("Resize Done, Download the Image");
});
